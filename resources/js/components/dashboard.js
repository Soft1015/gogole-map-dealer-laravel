...
const Dashboard = () => {
  ...
  const getAllData = (param) => {
    ...
    axios.post('/getType', { param: param })
      .then(function (response) {
        const data = response.data.mapInfo;
        let detials = [];
        for (let i = 0; i < data.length; i++) {
          var xml = new XMLParser().parseFromString(data[i].data);
          const results = xml.getElementsByTagName('result');
          for (let j = 0; j < results.length; j++) {
            const obj = {
              name: results[j].getElementsByTagName('name')[0].value,
              accountno: results[j].getElementsByTagName('accountno')[0].value,
              postcode: results[j].getElementsByTagName('postcode')[0].value,
              latitude: parseFloat(results[j].getElementsByTagName('latitude')[0].value),
              longitude: parseFloat(results[j].getElementsByTagName('longitude')[0].value),
              email: results[j].getElementsByTagName('email')[0] ? results[j].getElementsByTagName('email')[0].value : '',
              phone: results[j].getElementsByTagName('phone')[0] ? results[j].getElementsByTagName('phone')[0].value : '',
              website: results[j].getElementsByTagName('website')[0] ? results[j].getElementsByTagName('website')[0].value : '',
              type: data[i].type[0] ? data[i].type[0] : "",
        ...
        setMarkInfo(detials);
        setDealInfo(detials);
        setCurrType(response.data.type.type[0]);
        setLoadingState(false);
      })
      .catch(function (error) {
        console.log(error); setLoadingState(false);
      });
  };
  ...
  const getBrand = async () => {
    ...
    axios.get('/getBrand').then(response => {
      ...
      const arr = xml.getElementsByTagName('brand');
      for (let i = 0; i < arr.length; i++) {
        brand = brand.concat(arr[i].value);
      }
      setBrand(brand);
      setCurrBrand(brand[0]);
    }).catch((error) => {
      console.error(error);
    });
  };
  ...
  const sendSMS = (accountNo, phone) => {
    ...
    setLoadingState(true);
    axios.post('/sendSMS', { accountNo: accountNo, phone: phone })
      .then(function (response) {
